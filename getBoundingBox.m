...
%Kab Lab
%3/25/21
%Calls Ben's STL Generator to get bounding box

function sideLength = getBoundingBox(dataT, ID, testT)
    ...
    if testT.STL
        ...
        if capHeight == 0
            pythonScript = char(strcat({' '}, basePath, 'SingleLineBen\2DCurves\TISC_cl_SquareSide.py'));
        ...
        indexStart = strfind(cmdout,'Square Side Length:') + 19;
        indexEnd = strfind(cmdout(indexStart:end),':')+indexStart-2;
        sideLength = str2num(cmdout(indexStart:indexEnd));
